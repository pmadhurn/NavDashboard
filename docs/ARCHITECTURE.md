...
| **2b** | **Legacy authz migration. Done 2026-08-11, and superseded in shape.** Section x level was replaced outright by per-capability permission keys — see §4b below. All 116 legacy guards deleted; all 247 operations mapped. | This is the pass that "will break things" | verify-authz 21/21 as a non-admin. Fixes AUDIT §5.3 #1, #2 |
...
| **8** | **Admin engine completion. Done 2026-08-11** (`/settings/access`: per-user permission matrix, role editor, session control). (`/settings/roles`, `/settings/permissions` matrix editor) | Editing a permission model that isn't finished until step 2 is pointless; doing it last means the UI is built against the final shape | Admin can define a role and grant per-module, per-scope access |
...
**2026-08-11 — the permission model changed shape: section x level x scope → per-capability keys.** §4's `(section, level, scope)` triple is superseded for the *level* dimension. The user asked for control "down to each and every thing… I can only make user see dashboard to making him admin", which a four-level ladder per section cannot express: `devices: EDIT` is one grant covering create, update and status changes with no way to separate them.

> **What replaced it:** ~122 permission keys of the form `<group>.<action>` (`devices.read`, `devices.delete`, `finance.settle`, `backup.restore`), catalogued in `core/authz_catalog.py`, one per capability. `roles`/`role_permissions`/`user_roles` carry the bulk of a grant; `user_permission_overrides` adds ALLOW/DENY per person. **Rejected:** one key per operation (239 checkboxes — technically maximal, practically unusable); and keeping levels alongside keys (two systems again, which is the defect this whole step exists to close).
>
> **The scope dimension survives unchanged.** It answers a different question — *whose* records — and the key model does not carry it. It is now applied through `scoped_by_body()` / `scoped_by_path()` rather than through `require_permission`.

**2026-08-11 — enforcement moved from per-route dependencies to one app-level dependency.** §4.3 proposed `Depends(require_permission(...))` on each route. That is opt-in, and the property that matters — *no operation escapes* — is a statement about the whole route table that a per-route decorator cannot make.

> **What replaced it:** a single dependency registered on the `FastAPI(...)` constructor, which resolves the matched route, looks its permission up in `core/authz_endpoints.py`, and denies if absent. `assert_full_coverage()` runs at startup and refuses to boot if any registered operation is unmapped, so a new endpoint added without a permission stops the app instead of shipping unguarded. **Rejected:** middleware (runs before routing resolves, so it cannot see which endpoint matched, only the raw path).
>
> **Two implementation traps, both load-bearing.** (1) This FastAPI version reports `route.path` **without** the router prefix — `"/"` for the device list — so a path-keyed registry matches nothing and denies everything; the map is keyed by endpoint `module.function` instead. (2) Appending to `app.router.dependencies` **after** routes are registered silently does nothing, because dependants are baked in at registration; it must be passed to the constructor.

**2026-08-11 — sessions added, which §3 did not propose.** A JWT was valid until expiry with no way to end it, so "remove this person's access" meant "in up to `JWT_EXPIRY_MINUTES`". `user_sessions` ties a token to a row via a `jti` claim checked on every request. Session state is deliberately **not** cached (unlike the 30s permission-map cache), so revocation is immediate.
