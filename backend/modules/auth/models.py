...
class UserScope(Base):
    """Row-ownership narrowing per section: SELF | TEAM | ALL.

    Separate from `user_permission_overrides` because scope is per *section*
    while permissions are per *capability* — a team lead's TEAM scope on
    attendance applies to reading, editing and approving alike, and repeating
    it on every attendance key would be three places to get it wrong.

    Absent = the role default, and absent from that = ALL. Narrowing is always
    opt-in, so adding a scope check can never silently take access away.
    """

    __tablename__ = "user_scopes"
    __table_args__ = (UniqueConstraint("user_id", "section", name="uq_user_scope"),)
