...
async def seed_pairs(
    ...
    # Fetch all non-deleted, unassigned couples
    couples_stmt = (
        select(Couple)
        .where(Couple.deleted_at.is_(None))
        .order_by(Couple.created_at)
    )
    couples_result = await db.execute(couples_stmt)
    all_couples = list(couples_result.scalars().all())

    if len(all_couples) < 4:
        raise BadRequestException(
            f"Need at least 4 couples to seed pairs, found {len(all_couples)}. "
            "Please seed couples first (POST /api/v1/couples/seed)."
        )

    # Try to find by name first, fall back to positional
    couples_by_name: dict[str, Couple] = {c.name: c for c in all_couples}

    a1 = couples_by_name.get("Couple A1")
    a2 = couples_by_name.get("Couple A2")
    b1 = couples_by_name.get("Couple B1")
    b2 = couples_by_name.get("Couple B2")

    # Fallback: use first 4 couples if names don't match
    if not all([a1, a2, b1, b2]):
        # Filter to only unassigned couples
        unassigned = [c for c in all_couples if c.pair_id is None]
        if len(unassigned) < 4:
            raise BadRequestException(
                f"Need at least 4 unassigned couples to seed pairs, "
                f"found {len(unassigned)}. Please ensure couples exist and "
                "are not already assigned to pairs."
            )
        a1 = unassigned[0]
        a2 = unassigned[1]
        b1 = unassigned[2]
    ...
    # Verify none are already assigned
    for c in [a1, a2, b1, b2]:
        assert c is not None
        if c.pair_id is not None:
            raise ConflictException(
                f"Couple '{c.name}' is already assigned to a pair"
            )

    # Fetch personnel for handling_person
    personnel_stmt = (
        select(Person)
    ...
    # Pair 01: first two couples
    assert a1 is not None and a2 is not None
    pair1_in = PairCreate(
        name="Pair 01",
        couple_ids=[a1.id, a2.id],
        handling_person_id=person_ids[0] if len(person_ids) > 0 else None,
        notes="Primary rooftop pair",
    )
    pair1_resp = await create_pair(db, pair1_in, user_id)
    results.append(pair1_resp)

    # Pair 02: next two couples
    assert b1 is not None and b2 is not None
    pair2_in = PairCreate(
        name="Pair 02",
        couple_ids=[b1.id, b2.id],
        handling_person_id=person_ids[1] if len(person_ids) > 1 else None,
        notes="Secondary pair - includes faulty couple",
    )
    pair2_resp = await create_pair(db, pair2_in, user_id)
    results.append(pair2_resp)
